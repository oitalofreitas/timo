...
const AUTH_DIR = path_1.default.join("/tmp", "whatsapp-auth");
if (!fs_1.default.existsSync(AUTH_DIR)) {
    fs_1.default.mkdirSync(AUTH_DIR, { recursive: true });
}
async function loadAuthFromDB(connectionId) {
    try {
        ...
        const sessionPath = path_1.default.join(AUTH_DIR, connectionId);
        try {
            fs_1.default.writeFileSync(path_1.default.join(sessionPath, "creds.json"), connection.authData);
...
async function saveAuthToDB(connectionId, sessionPath) {
    try {
        const credsPath = path_1.default.join(sessionPath, "creds.json");
        if (fs_1.default.existsSync(credsPath)) {
            const authData = fs_1.default.readFileSync(credsPath);
...
async function initializeWhatsApp(connectionId, workspaceId) {
    ...
    try {
        ...
        if (!sessionPath) {
            sessionPath = path_1.default.join(AUTH_DIR, connectionId);
...
app.get("/api/whatsapp/events/:connectionId", (req, res) => {
    ...
    const session = sessions.get(sessionId);
    if (session) {
        res.write(`data: ${JSON.stringify({ type: "state", status: session.status, qrCode: session.qrCode })}\n\n`);
    }
...
function broadcastQRCode(connectionId, qrCode) {
    const clients = sseClients.get(connectionId);
...
function broadcastStatus(connectionId, status) {
    const clients = sseClients.get(connectionId);
    if (clients && clients.length > 0) {
